...
class DataObject {
  ...
  public static $cacheObj = array(); //array of singletons for all subclasses
  ...
  //--------------------------------------------------------------------------
  {
    return intval(database_query("SELECT JULIANDAY(?)-JULIANDAY(?)",array($endDate,$startDate))->fetchColumn(0));
  }
  //--------------------------------------------------------------------------
  public static function dateIsBefore($date,$compareTo)
  //--------------------------------------------------------------------------
  {
    return intval(database_query("SELECT JULIANDAY(?)-JULIANDAY(?)",array($date,$compareTo))->fetchColumn(0)) < 0;
  }
  //--------------------------------------------------------------------------
      ...
      foreach ($this->requiredFields as $field)
        if (!isset($this->$field) || $this->$field === '')
          if (!$this->isImage($field) && !Media::isFileUploaded($field.'_file'))
            throw new ValidationException(ValidationException::REQUIRED_FIELD,get($this->fieldLabels,$field,$field));
    }
    if (isset($this->dateFields)) {
      foreach ($this->dateFields as $field)
    ...
    if (isset($this->dateTimeFields)) {
      foreach ($this->dateTimeFields as $field)
        if (isset($this->$field) && $this->$field !== '' && !self::validateDateTime($this->$field))
          throw new ValidationException(ValidationException::INVALID_DATETIME,get($this->fieldLabels,$field,$field));
    }
  }
  //--------------------------------------------------------------------------
...
/*      if ($this->isDate($field))
  ...
  //--------------------------------------------------------------------------
  {
    ...
    $cache->readQuery = 'SELECT '.$this->getQueryFieldNames()." FROM $this->tableName $this->prefix WHERE $keyName=?";
    $record = database_query($cache->readQuery,array($keyValue))->fetch(PDO::FETCH_ASSOC);
    if ($record !== FALSE) {
...
//            }
    ...
    try {
      ...
      if (isset($this->fk)) {
        foreach ($this->fk as $fk) {
          ...
          $list = $data->queryBy("$fk->field=?",$data->primaryKeyName,null,array($this->getPrimaryKeyValue()))->fetchAll(PDO::FETCH_NUM);
          for ($i = 0; $i < count($list); ++$i) {
            ...
            switch ($fk->action) {
              ...
              case FKDeleteAction::DENY:
                throw new DataModelException($this,"This data object is being referenced by another one of type $fk->class and cannot be deleted.");
            }
          }
        }
      }
  ...
  //--------------------------------------------------------------------------
  {
    ...
    foreach ($fields as $fieldInfo) {
      ...
      else if (is_string($value)) $output .= '"'.str_replace(array("\r","\n",'"'),array('\n','','\"'),$value).'"';
      else $output .= 'null';
    }
    return $output . '}';
  }
